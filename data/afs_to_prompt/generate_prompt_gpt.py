...
def grd_section(G, exts, json_dir, f):
    ...
    with open(prompt_dir / (file_name + "_grd" + ".txt"), 'w') as f_write:
        ...
        IN_OUT_recursion(set_in, set_out, set_rest, ext_grd, ext_grd_out, ext_grd_und, G, f_write, prompt_dir,
                         file_name)

    if random.random() < 0.25:
        with open(prompt_dir / (file_name + "_grd" + ".txt"), 'r') as f_read:
            all_text = f_read.read()
            all_text = gpt_polish(all_text, prompt_dir, file_name + "_grd" + ".txt")

        with open(prompt_dir / (file_name + "_grd" + ".txt"), 'w') as f_write:
            print(all_text, file=f_write)
...
def IN_OUT_recursion(set_in: set, set_out: set, set_rest: set, ext_grd, ext_grd_out, ext_grd_und, G, f_write,
    ...
    IN_OUT_recursion(set_in, set_out, set_rest, ext_grd, ext_grd_out, ext_grd_und, G, f_write, prompt_dir, file_name)
...
def com_section(G, exts, json_dir, f):
    ...
    prompt_dir = json_dir / "prompt_com"
    prompt_dir.mkdir(parents=True, exist_ok=True)
    file_name = f.stem
    with open(prompt_dir / (file_name + "_com" + ".txt"), 'w') as f_write:
        ...
        if len(set_und) == 0:
            ...
            if random.random() < 0.25:
                with open(prompt_dir / (file_name + "_com" + ".txt"), 'r') as f_read:
                    all_text = f_read.read()
                    all_text = gpt_polish(all_text, prompt_dir, file_name + "_com" + ".txt")
...
def gpt_polish(text, prompt_dir, file_name, max_retries=5, retry_delay=3):
    system_msg = '''Following is one data sample generated using a fixed template in my QA dataset.
    Now I want to enhance data diversity. Please modify the natural language part in QA and illustrate the problem and answer in another way.
    You can change the order of the illustration, change the words and tones, modify the algorithm steps.
    You CANNOT modify the graph description and final JSON answer.
    Return the text after enhacement, don't return other thing.
    ...
    while retries < max_retries:
        try:
            rsp = gpt.completion(messages)
            return rsp
        except Exception as e:
            retries += 1
            if retries < max_retries:
                print(f"GPT API request failed, retrying in {retry_delay} seconds... (attempt {retries}/{max_retries})")
                time.sleep(retry_delay)
            else:
                print(f"GPT API request failed after {max_retries} attempts. Saving current state for recovery.")
                save_state_for_recovery(prompt_dir, file_name)
                raise e


def save_state_for_recovery(prompt_dir, file_name):
    # Save the current state (text and retries count) to a file or database for future recovery
    recovery_data = {
        "dir": prompt_dir,
        'file_name': file_name,
    }
    with open("./recovery_state.json", "a") as f:
        json.dump(recovery_data, f)


def load_state_for_recovery():
    # Load the saved state from a file or database
    try:
        with open("recovery_state.json", "r") as f:
            recovery_data = json.load(f)
        return recovery_data["text"], recovery_data["retries"]
...
def process_dataset(dataset_name):
    ...
    grd_dir = json_dir / "prompt_grd"
    com_dir = json_dir / "prompt_com"
    ...
    with Pool(1) as pool:
        list(tqdm(pool.starmap(process_file, [(f, json_dir) for f in filtered_af_files]), total=len(filtered_af_files)))


if __name__ == '__main__':
    t = time.time()
    random.seed(42)  # for reproducibility
    train = [f"train-{i}" for i in range(6, 26)]
    test = [f"test-{i}" for i in range(6, 26)]
    datasets = train + test
    print(datasets)
